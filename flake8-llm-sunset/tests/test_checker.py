...
def test_warns_one_calendar_month_before_shutdown():
    ...
    assert len(errors) == 1
    assert errors[0][2] == (
        "LLS002 Gemini model 'gemini-2.5-flash' shuts down on 2026-10-16 "
        "(30 days remaining)"
    )
...
def test_warns_for_already_shutdown_model():
    ...
    assert errors[0][2] == (
        "LLS001 Gemini model 'gemini-2.0-flash' was shut down on 2026-06-01"
    )
...
def test_uses_upcoming_code_until_day_before_shutdown():
    ...
    assert errors[0][2].startswith("LLS002 ")
...
def test_uses_shutdown_code_on_shutdown_date():
    ...
    assert errors[0][2] == (
        "LLS001 Gemini model 'gemini-2.5-flash' was shut down on 2026-10-16"
    )
...
def test_distinguishes_earliest_possible_shutdown_date():
    ...
    assert errors[0][2] == (
        "LLS002 Gemini model 'gemini-3.5-flash' may shut down on or after 2027-05-19"
    )
...
def test_uses_shutdown_code_after_earliest_possible_shutdown_date():
    ...
    assert errors[0][2] == (
        "LLS001 Gemini model 'gemini-3.5-flash' may shut down on or after 2027-05-19"
    )
...
def test_warns_for_veo_model_from_expanded_markdown_section():
    ...
    assert errors[0][2] == (
        "LLS002 Veo model 'veo-3.0-generate-001' shuts down on 2026-06-30 "
        "(29 days remaining)"
    )
...
def test_distinguishes_earliest_possible_veo_shutdown_date():
    ...
    assert errors[0][2] == (
        "LLS002 Veo model 'veo-3.1-fast-generate-001' may shut down on or after "
        "2026-11-17"
    )
...
def test_warns_for_embedding_model():
    ...
    assert errors[0][2] == (
        "LLS002 Google model 'text-embedding-005' shuts down on 2027-04-01 "
        "(31 days remaining)"
    )
...
def test_warns_for_retired_model_from_expanded_markdown_section():
    ...
    assert errors[0][2] == (
        "LLS001 Google model 'textembedding-gecko@003' was shut down on 2025-05-24"
    )
